...
ハードウェアの設定を行います。  
後でも設定できますが、変更個所は CPU 設定の
- "ハードウェア仮想化" : "ハードウェア アシストによる仮想化をゲスト OS に公開"
です。必ずチェックを入れてください。
"ハードウェア仮想化" : "ハードウェア アシストによる仮想化をゲスト OS に公開" を有効にする事で、仮想マシンの構成ファイル .vmx ファイルに
vhv.enable = "TRUE"
が追記されます。

親となる vSphere ESXi のバージョンが 6.5 以前の場合は "I/O MMU" を有効化します。6.7 以降では不要です。
- I/O MMU : "有効"
